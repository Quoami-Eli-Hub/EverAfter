...
export default async function Dashboard(){
  ...
  return <div className="app-shell"><DashboardNav active="Overview"/><main className="app-main">
    <div className="page-heading"><div><p className="eyebrow">Active workspace · {role}</p><h1>{event.title}</h1><p>{event.event_type==="wedding"?"Wedding":"Memorial"} · {event.event_date??"Date not set"} · {events.length} accessible {events.length===1?"event":"events"}</p></div><div className="button-row"><Link href="/dashboard/events" className="button panel-button">Switch event</Link><Link href={publicHref} className="button panel-button">Preview page ↗</Link>{["owner","planner"].includes(role)&&<form action={setPublicationStatus}><button className="button button-dark" name="publish" value={event.status==="published"?"false":"true"}>{event.status==="published"?"Unpublish":"Publish page"}</button></form>}</div></div>
