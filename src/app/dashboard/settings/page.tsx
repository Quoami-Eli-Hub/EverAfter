...
export default async function Settings({searchParams}:{searchParams:Promise<{password?:string;message?:string}>}) {
  ...
  return <div className="app-shell"><DashboardNav active="Settings"/><main className="app-main">
    ...
    <nav className="settings-jumps" aria-label="Settings sections"><a href="#invitation">Invitation</a><a href="#appearance">Appearance</a><a href="#music">Music</a><a href="#event-details">Details & sharing</a><a href="#privacy">Password</a></nav>
    <div id="invitation">{<InvitationEditor theme={event.theme_key} memorial={event.event_type==="memorial"} initial={event.invitation_cover} names={event.title} date={invitationDate(event.event_date)} palette={event.color_key} font={event.font_key} eventId={event.id} canEdit={["owner","planner"].includes(role)}/>}</div>
    <MusicEditor key={event.id} eventId={event.id} premium={event.plan_code==="premium"&&event.plan_paid===true} canEdit={["owner","planner"].includes(role)} hasMusic={Boolean(event.music_path)}/>
    <div className="settings-layout">
    <form id="appearance" action={updateEvent} className="settings-workspace">
      <AppearancePicker eventType={event.event_type==="memorial"?"memorial":"wedding"} initialTheme={event.theme_key} initialColor={event.color_key} initialFont={event.font_key} premium={event.plan_code==="premium"&&event.plan_paid===true}/>
