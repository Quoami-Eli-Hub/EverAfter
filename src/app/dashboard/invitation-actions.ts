...
export async function saveInvitationCover(_previous: { ok: boolean; message: string }, form: FormData) {
  ...
  if (!["owner", "planner"].includes(role) || event.event_type !== "wedding") return { ok: false, message: "Only the wedding owner or planner can change this invitation." };
  if (Number(form.get("eventId")) !== event.id) return { ok: false, message: "Your active event changed. Refresh before saving." };
  const settings = invitationSettings({ enabled: form.get("enabled") === "on", names: form.get("names"), date: form.get("date"), message: form.get("message"), button: form.get("button"), emblem: form.get("emblem") });
