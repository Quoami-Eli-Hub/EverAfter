...
const templateCopy={
  wedding:{classic:["Classic","Timeless, graceful and ceremony-led"],garden:["Garden","Soft, organic and photography-forward"],editorial:["Editorial","Bold, modern and magazine-inspired"],cinematic:["Cinematic","Immersive, dramatic and image-led"],minimalist:["Minimal","Quiet luxury with generous space"],romance:["Romance","Blush stationery, plum accents and a romantic photo story"]},
  memorial:{classic:["Legacy","Dignified, timeless and family-centred"],garden:["Serene","Gentle, natural and reflective"],editorial:["Tribute journal","Modern, editorial and story-led"],cinematic:["Remembrance film","Immersive, emotional and image-led"],minimalist:["Stillness","Restrained, peaceful and contemplative"],romance:["Keepsake","Soft stationery and a heartfelt photo story"]}
} as const;
const palettes={
...
export function AppearancePicker({eventType,initialTheme,initialColor,initialFont,premium}:{eventType:EventType;initialTheme:string;initialColor:string;initialFont:string;premium:boolean}){
  ...
  return <section className={`appearance-builder preview-theme-${theme} preview-palette-${color} preview-font-${font}`}>
    <div className="appearance-heading"><div><p className="eyebrow">Your page, your style</p><h2>Find your look</h2><p>Choose a layout, colour and type style. Your story and photographs stay with you.</p></div><div className="appearance-live"><small>Live style preview</small><div className="mini-page"><i/><div><span>{eventType==="wedding"?"SAVE THE DATE":"IN LOVING MEMORY"}</span><b>{eventType==="wedding"?"Ama & Kojo":"Remembering Kofi"}</b><em/></div></div></div></div>
    <fieldset><legend>Template</legend><div className="template-options">{Object.entries(templateCopy[eventType]).map(([key,value])=>{const locked=["cinematic","minimalist","romance"].includes(key)&&!premium;return <label className={`template-option${locked?" locked-template":""}`} key={key}><input type="radio" name="theme" value={key} checked={theme===key} disabled={locked} onChange={()=>setTheme(key)}/><span className={`template-thumb thumb-${key}`}><i/><i/><i/></span><b>{value[0]}</b><small>{value[1]}</small><em>{locked?"Premium":theme===key?"Selected":"Choose"}</em></label>})}</div></fieldset>
    <p><a href="/demo/romance" target="_blank" rel="noreferrer">Open the Romance template preview ↗</a></p>
