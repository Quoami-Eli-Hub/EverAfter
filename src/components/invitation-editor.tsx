...
export function InvitationEditor({ initial, names, date, palette, font, eventId, canEdit }: { initial: unknown; names: string; date: string; palette: string; font: string; eventId: number; canEdit: boolean }) {
  const [settings, setSettings] = useState(() => invitationSettings(initial));
  ...
  return <section className="panel invitation-studio"><p className="eyebrow">The first impression</p><h2>Invitation cover</h2><p>Welcome guests with a personal invitation that opens to reveal your wedding website. Saving applies this cover to your current page.</p>
    <div className="invitation-studio-grid"><form action={action} className="manage-form">
      ...
      <fieldset disabled={!canEdit || pending} style={{ border: 0, padding: 0, display: "grid", gap: 16 }}>
        <label className="check-row"><input type="checkbox" name="enabled" checked={settings.enabled} onChange={e => update("enabled", e.target.checked)}/> Show invitation before the website</label>
        <label>Couple names<input name="names" maxLength={120} value={settings.names} placeholder={names} onChange={e => update("names", e.target.value)}/></label>
        ...
        <label>Emblem<select name="emblem" value={settings.emblem} onChange={e => update("emblem", e.target.value as InvitationSettings["emblem"])}><option value="initials">Couple initials</option><option value="flower">Sunburst flower</option><option value="none">No emblem</option></select></label>
      ...
      <div className={`invitation-preview-frame ${size} palette-${palette} font-${font}`}><InvitationCover key={replay} settings={settings} names={names} date={date} preview/></div>
      <p><small>Preview uses your saved palette and typography. The cover is {settings.enabled ? "enabled" : "disabled"}; save to apply changes.</small></p>
    </div></div>
  </section>;
}
